...
from .utils import nested_method
import datetime
...
class CeleryClient(Singleton):
        ...
        for worker_name, all_tasks in response.items():
            ...
            for task in all_tasks:
                ...
                registered_tasks.append(
                    {"task_name": task.split("[")[0], "queue": queue}
                )
    ...
    def execute(self, command, parameter):
        def run(*args):
            ...
            return {
                "status": "success",
                "message": "Task %s has been assigned task id is %s."
                % (task_name, response),
            }
        ...
        def revoke(*args):
            ...
            response = ctrl.revoke(
                task_id, terminate=True, signal="SIGKILL", reply=True,
            )
            if (
                "ok" in response[0][destination]
                and "tasks unknown" not in response[0][destination]["ok"]
            ):
                response = {
                    "status": "success",
                    "message": "Task %s on server %s has been revoked."
                    % (task_id, destination),
                }
            else:
                response = {
                    "status": "failure",
                    "message": "Task %s on server %s does not exist."
                    % (task_id, destination),
                }
            return response
        ...
        def terminate(*args):
            ...
            if (
                ...
                response = {
                    "status": "success",
                    "message": "Task %s on server %s has been terminated."
                    % (task_id, destination),
                }
            else:
                response = {
        ...
        def shutdown(*args):
            ctrl = args[0]
            destination = args[1]["destination"]
            response = ctrl.broadcast("shutdown", destination=[destination], reply=True)
            response = {
                "status": "success",
                "message": "Worker on %s will shutdown." % (destination),
            }
            return response
        ...
        def restart(*args):
            ...
            if response and "ok" in response[0][destination]:
                response = {
                    "status": "success",
                    "message": "Worker on %s reload started." % (destination),
                }
            elif response and "error" in response[0][destination]:
                response = {
                    "status": "failure",
                    "message": "Worker on %s reload failure. Reason:%s"
        ...
        def ping(*args):
            ...
            if response and "ok" in response[0][destination]:
                response = {
                    "status": "success",
                    "message": "Server %s is online." % (destination),
                }
            else:
                response = {
                    "status": "failure",
                    "message": "Server %s is offline." % (destination),
                }
            return response
        ...
        def time_limit(*args):
            ...
            if method == "soft":
                ...
                response = ctrl.time_limit(
                    taskname, soft=soft, reply=True, destination=[destination]
                )
            else:
                hard = int(args[1]["time_limit_hard"])
                response = ctrl.time_limit(
                    taskname, hard=hard, reply=True, destination=[destination]
                )
            if len(response) != 0 and response[0][destination].has_key("ok"):
                response = {
                    "status": "success",
        ...
        def poolgrow(*args):
            ...
            destination = args[1]["destination"]
            response = ctrl.pool_grow(
                n=grown_num, reply=True, destination=[destination]
            )
            if (
                len(response) != 0
                and response[0][destination].has_key("ok")
                and "pool will grow" in response[0][destination]["ok"]
            ):
                response = {
                    "status": "success",
                    "message": "Pool on server %s will grow %d pool."
                    % (destination, grown_num),
                }
            elif len(response) != 0 and response[0][destination].has_key("error"):
                response = {
                    "status": "failure",
                    "message": "Parameter or errors happen, Please check,reason:%s"
        ...
        def poolshrink(*args):
            ...
            destination = args[1]["destination"]
            response = ctrl.pool_shrink(
                n=shrink_num, reply=True, destination=[destination]
            )
            if (
                len(response) != 0
                and response[0][destination].has_key("error")
                and "Can't shrink pool. All processes busy!"
                in response[0][destination]["error"]
            ):
                response = {
                    "status": "failure",
                ...
                len(response) != 0
                and response[0][destination].has_key("ok")
                and "pool will shrink" in response[0][destination]["ok"]
            ):
                response = {
                    "status": "success",
                    "message": "pool on server %s will shrink %d pool."
                    % (destination, shrink_num),
                }
            elif len(response) != 0 and response[0][destination].has_key("error"):
                response = {
                    "status": "failure",
